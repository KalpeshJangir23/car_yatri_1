...
class OutstationTripForm extends ConsumerStatefulWidget {
  const OutstationTripForm({super.key});

  @override
  _OutstationTripFormState createState() => _OutstationTripFormState();
}

class _OutstationTripFormState extends ConsumerState<OutstationTripForm> {
  ...
  Widget build(BuildContext context) {
    ...
    return Form(
      child: ListView(
        children: [
          _buildTripTypeButtons(formState),
          ...
          const SizedBox(height: 16),
          if (isRoundTrip) ...[
            const Text('Stop Points:', style: TextStyle(fontWeight: FontWeight.bold)),
            const SizedBox(height: 8),
            ..._buildStopPoints(formState),
            const SizedBox(height: 16),
          ],
  ...
  Widget _buildTripTypeButtons(Map<String, dynamic> formState) {
    var theme = AppTheme.lightTheme;
    return Row(
      children: [
        Expanded(
          child: ElevatedButton(
            onPressed: () {
              ref.read(outstationProvider.notifier).toggleTripType('one-way');
              ref.read(outstationProvider.notifier).clearStops();
            },
            style: ElevatedButton.styleFrom(
              backgroundColor: formState['tripType'] == 'one-way' ? theme.primaryColor : Colors.white,
              padding: const EdgeInsets.symmetric(vertical: 12),
            ),
            child: Text(
              'One-Way Trip',
              style: TextStyle(
                color: formState['tripType'] == 'one-way' ? Colors.white : theme.primaryColor,
              ),
            ),
          ),
        ),
        ...
        Expanded(
          child: ElevatedButton(
            onPressed: () {
              ref.read(outstationProvider.notifier).toggleTripType('round-way');
            },
            style: ElevatedButton.styleFrom(
              backgroundColor: formState['tripType'] == 'round-way' ? theme.primaryColor : Colors.white,
              padding: const EdgeInsets.symmetric(vertical: 12),
            ),
            child: Text(
              'Round-Way Trip',
              style: TextStyle(
                color: formState['tripType'] == 'one-way' ? theme.primaryColor : Colors.white,
              ),
            ),
          ),
        ),
  ...
  List<Widget> _buildStopPoints(Map<String, dynamic> formState) {
    return formState['stops'].map<Widget>((stop) {
      return Container(
        ...
        child: Row(
          children: [
            ...
            const SizedBox(width: 16),
            Expanded(
              child: Text(stop, style: const TextStyle(fontSize: 16)),
            ),
            IconButton(
              icon: const Icon(Icons.delete, color: Colors.red),
              onPressed: () {
                ref.read(outstationProvider.notifier).removeStop(stop);
              },
            ),
          ],
        ),
